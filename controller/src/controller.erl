...
init([]) ->
    ...
    % Check if etcd -dbase is up n' running
    pong=tcp:call(?ETCD_PUBLIC_IP,?ETCD_PUBLIC_PORT,
		  ?ETCD_LOCAL_IP,?ETCD_LOCAL_PORT,
		  {etcd,ping,[]}),

    [{_,ControllerServiceInfo}]=tcp:call(?ETCD_PUBLIC_IP,?ETCD_PUBLIC_PORT,
				 ?ETCD_LOCAL_IP,?ETCD_LOCAL_PORT,
				 {etcd,read,[{service_info,"controller","1.0.0"}]}),
    DnsPublic=ControllerServiceInfo#service_info.public,
    {DnsLocalIp,_}=ControllerServiceInfo#service_info.local,  
    DnsServiceInfo=#service_info
	{
	  status=started,
	  service_id="dns",
	  vsn="1.0.0",
	  public=ControllerServiceInfo#service_info.public,
	  local={DnsLocalIp,?DNS_LOCAL_PORT}
	},
        
    {ok,updated}=tcp:call(?ETCD_PUBLIC_IP,?ETCD_PUBLIC_PORT,
			  ?ETCD_LOCAL_IP,?ETCD_LOCAL_PORT,
			  {etcd,update,[{service_info,"dns","1.0.0"},DnsServiceInfo]}),
    
    [{_,MasterNodeInfo}]=tcp:call(?ETCD_PUBLIC_IP,?ETCD_PUBLIC_PORT,
				 ?ETCD_LOCAL_IP,?ETCD_LOCAL_PORT,
				 {etcd,read,[{node_info,"master"}]}),
    {MasterPublicIp,MasterPublicPort}=MasterNodeInfo#node_info.public,
    {MasterLocalIp,MasterLocalPort}=MasterNodeInfo#node_info.local,
    ok=tcp:call(MasterPublicIp,MasterPublicPort,
		MasterLocalIp,MasterLocalPort,
		{node,start_application,["dns","1.0.0"]}),
    % 
    
    
    % Start/restart dns function
% Lägg till att man kan ha egen time out på tcp och if_dns eftersom vissa
% tillfällen behöver inte ha längre tid
    % Ping etcd - secure it's started
    % if not - error msg - Fatal error - no dbase
    % Start dns , log
    % Update dns with etcd info
    % start log -> register dns
    % 
    % Set the sysem to "wanted state"
    % Update applications , services, and cluster status
    % Check applications , services and health på clustret
    % läs in nodes 
    % polla av vilks som finns
    % Starta poll process för nodes
    %starta poll process for services - inför en "ping"

    % Check if something is already started
    % start dns
    % starta log
    % starta api server
